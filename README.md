# ctest
[![CI/CD GitHub Actions](https://github.com/KorolLich/my-ctest/actions/workflows/test-action.yml/badge.svg)](https://github.com/KorolLich/my-ctest/actions/workflows/test-action.yml)
[![Coverage Status](https://coveralls.io/repos/github/KorolLich/my-ctest/badge.png)](https://coveralls.io/github/KorolLich/my-ctest)
[![Quality Gate](https://sonarcloud.io/api/project_badges/measure?project=KorolLich_my-ctest&metric=alert_status)](https://sonarcloud.io/dashboard?id=KorolLich_my-ctest)
[![Bugs](https://sonarcloud.io/api/project_badges/measure?project=KorolLich_my-ctest&metric=bugs)](https://sonarcloud.io/summary/new_code?id=KorolLich_my-ctest)
[![Code smells](https://sonarcloud.io/api/project_badges/measure?project=KorolLich_my-ctest&metric=code_smells)](https://sonarcloud.io/dashboard?id=KorolLich_my-ctest)
# План тестирования:

# Аттестационное тестирование

  - Тест А1 (положительный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу `./ctest fibonachi 4`
    - Ожидаемый результат:
        ```            
      	Фибоначи: 3
        ```               

  - Тест А2 (негативный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу без аргумента
    - Ожидаемый результат: 
      ```                  
     	Неверное количество аргументов
      ```                         

  - Тест А3 (негативный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с 2 аргументами: `./ctest fibonachi 4 5`
    - Ожидаемый результат: 
        ```                     
      	Использование: ./ctest fibonachi <num>
        ```                      

  - Тест А4 (положительный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом 0: `./ctest fibonachi 0`
    - Ожидаемый результат: 
        ```                           
      	Фибоначи: 0
        ```                           

  - Тест А5 (негативный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом -1
    - Ожидаемый результат: 
        ```                            
      	Фибоначи: 0
        ```                     

  - Тест А6 (негативный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом "bob"
    - Ожидаемый результат: 
        ```                       
      	Использование: ./ctest fibonachi <num>
        ```                     

  - Тест А7 (негативный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом 2.5
    - Ожидаемый результат: 
        ```                       
      	Использование: ./ctest fibonachi <num>
        ```                     

  - Тест А8 (положительный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом 10
    - Ожидаемый результат: 
        ```                       
      	Фибоначи: 55
        ```                     

  - Тест А9 (положительный)
    - Начальное состояние: Открытый терминал
    - Действие: Пользователь запускает программу с аргументом 25
    - Ожидаемый результат: 
        ```                       
      	Фибоначи: 75025
        ```

- Тест А10 (негативный)
  - Начальное состояние: Открытый терминал
  - Действие: Пользователь запускает программу с аргументом -1: `./ctest my_sqrt -1`
  - Ожидаемый результат: 
      ```                            
      Использование: ./ctest my_sqrt <a> <b> <c>
      ```

- Тест А11 (негативный)
  - Начальное состояние: Открытый терминал
  - Действие: Пользователь запускает программу с аргументом "bob": `./ctest my_sqrt bob`
  - Ожидаемый результат: 
      ```                       
      Использование: ./ctest my_sqrt <a> <b> <c>
      ```

- Тест А12 (негативный)
  - Начальное состояние: Открытый терминал
  - Действие: Пользователь запускает программу с аргументом 2.5: `./ctest my_sqrt 2.5`
  - Ожидаемый результат: 
      ```                       
      Использование: ./ctest my_sqrt <a> <b> <c>
      ```

- Тест А13 (положительный)
  - Начальное состояние: Открытый терминал
  - Действие: Пользователь запускает программу с аргументами 1.0 2.0 1.0: `./ctest my_sqrt 1.0 2.0 1.0`
  - Ожидаемый результат: 
      ```                       
      Результат для my_sqrt(1.00, 2.00, 1.00): root1=-1.000000, root2=-1.000000
      ```

- Тест А14 (положительный)
  - Начальное состояние: Открытый терминал
  - Действие: Пользователь запускает программу с аргументами 1.0 0.0 -1.0: `./ctest my_sqrt 1.0 0.0 -1.0`
  - Ожидаемый результат: 
      ```                       
      Результат для my_sqrt(1.00, 0.00, -1.00): root1=1.000000, root2=-1.000000
      ```
                                    

# Блочное тестирование (класс my_func)
<ol>
  <li>
    <h3>Метод int fibonachi(int num)</h3>
    <ol>
    	<li>
    	  <h4>Тест Б1.1 (положительный)</h4>
    	  <ul>
    	    <li>Входные данные: 4</li>
    	    <li>Ожидаемый результат: 3</li>
    	    <li>Действия:</li></br>
            - Вызвать функцию `fibonachi(4)`</li></br>
            - Проверить, что возвращенное значение равно 3</li></br>
          </ul>
      </li>
      <li>
        <h4>Тест Б1.2 (положительный)</h4>
        <ul>
          <li>Входные данные: 0</li>
          <li>Ожидаемый результат: 0</li>
          <li>Действия:</li></br>
              - Вызвать функцию `fibonachi(0)`</li></br>
              - Проверить, что возвращенное значение равно 0</li></br>
            </ul>
          </li>
          <li>
            <h4>Тест Б1.3 (положительный)</h4>
            <ul>
              <li>Входные данные: -1</li>
              <li>Ожидаемый результат: 0</li>
              <li>Действия:</li></br>
                - Вызвать функцию `fibonachi(-1)`</li></br>
                - Проверить, что возвращенное значение равно 0</li></br>
              </ul>
            </li>
         </ol>
        </li>
        <li>
          <h3>Метод Roots my_sqrt(double a, double b, double c)</h3>
          <ol>
            <li>
              <h4>Тест Б2.1 (положительный)</h4>
              <ul>
                <li>Входные данные: a=1.0, b=2.0, c=1.0</li>
                <li>Ожидаемый результат:</li></br>
                - root1 ≈ -1.0</li></br>
                - root2 ≈ -1.0</li></br>
                <li>Действия:</li></br>
                  - Вызвать функцию `my_sqrt(1.0, 2.0, 1.0)`</li></br>
                  - Проверить, что root1 и root2 близки к -1.0 с заданной точностью</li></br>
                </ul>
              </li>
              <li>
                <h4>Тест Б2.2 (положительный)</h4>
                <ul>
                  <li>Входные данные: a=2.3, b=-10.6, c=10.3</li>
                  <li>Ожидаемый результат:</li></br>
                  - root1 ≈ 3.21635552014</li></br>
                  - root2 ≈ 1.39234013202</li></br>
                  <li>Действия:</li></br>
                    - Вызвать функцию `my_sqrt(2.3, -10.6, 10.3)`</li></br>
                    - Проверить, что root1 и root2 близки к ожидаемым значениям с заданной точностью</li></br>
                  </ul>
                </li>
                <li>
                <h4>Тест Б2.3 (положительный)</h4>
                <ul>
                  <li>Входные данные: a=0.0, b=-10.6, c=10.3</li>
                  <li>Ожидаемый результат:</li></br>
                  - root1 ≈ 0.9716981132</li></br>
                  - root2 ≈ 0.9716981132</li></br>
                  <li>Действия:</li></br>
                    - Вызвать функцию `my_sqrt(0.0, -10.6, 10.3)`</li></br>
                    - Проверить, что root1 и root2 близки к ожидаемым значениям с заданной точностью</li></br>
                  </ul>
                </li>
                <li>
                  <h4>Тест Б2.4 (положительный)</h4>
                  <ul>
                    <li>Входные данные: a=1.0, b=0.0, c=-1.0</li>
                    <li>Ожидаемый результат:</li></br>
                    - root1 ≈ 1.0</li></br>
                    - root2 ≈ -1.0</li></br>
                    <li>Действия:</li></br>
                      - Вызвать функцию `my_sqrt(1.0, 0.0, -1.0)`</li></br>
                      - Проверить, что root1 и root2 близки к 1.0 и -1.0 с заданной точностью</li></br>
                    </ul>
                  </li>
                  <li>
                    <h4>Тест Б2.5 (негативный)</h4>
                    <ul>
                      <li>Входные данные: a=0.0, b=2.0, c=1.0</li>
                      <li>Ожидаемый результат: NULL (нет корней)</li>
                      <li>Действия:</li></br>
                        - Вызвать функцию `my_sqrt(0.0, 2.0, 1.0)`</li></br>
                        - Проверить, что result равен NULL</li></br>
                      </ul>
                    </li>
                    <li>
                      <h4>Тест Б2.6 (негативный)</h4>
                      <ul>
                        <li>Входные данные: a=1.0, b=2.0, c=2.0</li>
                        <li>Ожидаемый результат: NULL (нет корней)</li>
                        <li>Действия:</li></br>
                          - Вызвать функцию `my_sqrt(1.0, 2.0, 2.0)`</li></br>
                          - Проверить, что result равен NULL</li></br>
                        </ul>
                      </li>
                      <li>
                      <h4>Тест Б2.7 (негативный)</h4>
                      <ul>
                        <li>Входные данные: a=0.0, b=0.0, c=1.0</li>
                        <li>Ожидаемый результат: NULL (нет корней)</li>
                        <li>Действия:</li></br>
                          - Вызвать функцию `my_sqrt(0.0, 0.0, 1.0)`</li></br>
                          - Проверить, что result равен NULL</li></br>
                        </ul>
                      </li>
                    </ol>
                  </li>
                </ol>
              </ol>

# Интеграционное тестирование
<ol>
  <li>
    <h3>Тест И1</h3>
    <ul>
      <li>Методы: int fibonachi(int num), Roots my_sqrt(double a, double b, double c)</li>
      <li>Описание: Проверяем, можно ли использовать результат работы функции fibonachi в функции my_sqrt</li>
      <li>Входные данные для int fibonachi: 4 6 8</li>
      <li>Ожидаемый результат: 3 6 8</li>
      <li>Входные данные для Roots my_sqrt: 4 6 8</li>
      <li>Ожидаемый результат: result равен NULL (нет корней)</li>
    </ul>	
  </li>
</ol>
